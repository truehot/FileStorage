...
internal static class IndexEntrySerializer
{
    ...
    public static void Write(Span<byte> buffer, string table, Guid key, long dataOffset, int dataSize, long version)
    {
        ...
        BinaryPrimitives.WriteInt32LittleEndian(buffer[TableLenOffset..], len);

        key.TryWriteBytes(buffer.Slice(GuidOffset, GuidSize));

        BinaryPrimitives.WriteInt64LittleEndian(buffer[DataOffsetPos..], dataOffset);
        BinaryPrimitives.WriteInt32LittleEndian(buffer[DataSizePos..], dataSize);
        BinaryPrimitives.WriteInt64LittleEndian(buffer[VersionPos..], version);
    }
    ...
    public static Guid ReadKey(ReadOnlySpan<byte> buffer) =>
        new(buffer.Slice(GuidOffset, GuidSize));

    public static long ReadDataOffset(ReadOnlySpan<byte> buffer) =>
        BinaryPrimitives.ReadInt64LittleEndian(buffer[DataOffsetPos..]);

    public static int ReadDataSize(ReadOnlySpan<byte> buffer) =>
        BinaryPrimitives.ReadInt32LittleEndian(buffer[DataSizePos..]);

    public static long ReadVersion(ReadOnlySpan<byte> buffer) =>
        BinaryPrimitives.ReadInt64LittleEndian(buffer[VersionPos..]);

    public static string ReadTableName(ReadOnlySpan<byte> buffer)
    {
        int tableLen = BinaryPrimitives.ReadInt32LittleEndian(buffer[TableLenOffset..]);
    ...
    public static bool TableEquals(ReadOnlySpan<byte> buffer, string table)
    {
        ...
        return storedBytes.SequenceEqual(tableBytes[..written]);
    }
}